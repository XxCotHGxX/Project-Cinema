  ...
  return (
    <div className="relative min-h-screen bg-cinema-black text-white">
      ...
      <main className="px-12 -mt-16 relative z-20 space-y-12 pb-20">
        <VideoRow title="Library" movies={movies} />
        <VideoRow title="Trending Now" movies={movies} />
      </main>
    </div>
  );
}

const VideoRow: React.FC<{ title: string, movies: Movie[] }> = ({ title, movies }) => {
  return (
    <div className="space-y-4">
      <h3 className="text-xl font-bold text-gray-200">{title}</h3>
      <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
        {movies.map((movie, index) => (
          <div key={index} className="relative flex-none w-[250px] aspect-video bg-cinema-gray rounded overflow-hidden cursor-pointer group hover:scale-110 transition duration-300 z-0 hover:z-50">
